...
def build_rule_map(value):
    values = [1, 2, 4, 8, 16, 32, 64, 128]
    rules = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']
    rule_map = {}
    lvalue = value
    for val, rule in zip(reversed(values), reversed(rules)):
        if lvalue >= val:
            lvalue = lvalue - val
            rule_map[rule] = 'x'
        else:
...
def build_generation(rules, first_generation):
    new_gen = ''
    n = len(first_generation)
    for i in range(n):
        pattern = first_generation[i-1] + first_generation[i] + first_generation[(i+1)%n]
        new_gen = new_gen + rules[pattern]
    return new_gen

def cellular_automaton(first_gen, value, gens):
    rules = build_rule_map(value)
    next_generation = first_gen
    while gens > 0:
        next_generation = build_generation(rules, next_generation)
        gens -= 1
