...
def involved(courses, person):
    inv = {}
    for key in courses:
        for course_name, course_content in courses[key].iteritems():
            for course_item, course_item_value in course_content.iteritems():
                if course_item_value == person:
                    if key not in inv:
                        inv[key] = []
                    inv[key].append(course_name)
    return inv
